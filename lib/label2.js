import { q as queryElementRoots, b as closestElementCrossShadowBoundary } from "./index.js";
/*!
...
const findLabelForComponent = (componentEl) => {
  ...
  if (!parentLabel || hasAncestorCustomElements(parentLabel, componentEl)) {
    return null;
  }
  return parentLabel;
};
function hasAncestorCustomElements(label, componentEl) {
  ...
  const ancestorCustomElements = traversedElements.filter((el) => el !== componentEl && el !== label).filter((el) => {
    var _a;
    return (_a = el.tagName) === null || _a === void 0 ? void 0 : _a.includes("-");
  });

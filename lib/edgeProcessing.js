...
import { o } from "./glUtil.js";
import { bC as m$2, oa as p$3, ob as n, aS as o$1, bg as r$1, bf as P, bb as _$1, eS as l$1, dq as H, b9 as e$1, bE as z, a1 as n$1, aR as x, jQ as t$1, be as u$3 } from "./index.js";
...
const p$2 = -1;
var u$2;
function h$1(n$12, i, l2, f2 = N) {
  const g2 = n$12.vertices.position, m2 = n$12.vertices.componentIndex, u2 = m$2(f2.anglePlanar), h2 = m$2(f2.angleSignificantEdge), V2 = Math.cos(h2), S2 = Math.cos(u2), j = I.edge, x$1 = j.position0, E2 = j.position1, D = j.faceNormal0, F = j.faceNormal1, L = y$1(n$12), U = w$1(n$12), b = U.length / 4, k = i.allocate(b);
  let C = 0;
  const H2 = b, K = l2.allocate(H2);
  let M = 0, O2 = 0, P$1 = 0;
  const T2 = p$3(0, b), q = new Float32Array(b);
  n(q, (e2, t2, n2) => {
    g2.getVec(U[4 * t2 + 0], x$1), g2.getVec(U[4 * t2 + 1], E2), n2[t2] = x(x$1, E2);
  }), T2.sort((e2, t2) => q[t2] - q[e2]);
  const z2 = new Array(), B = new Array();
  for (let e2 = 0; e2 < b; e2++) {
    const t2 = T2[e2], n2 = q[t2], o2 = U[4 * t2 + 0], r2 = U[4 * t2 + 1], f3 = U[4 * t2 + 2], h3 = U[4 * t2 + 3], w2 = h3 === p$2;
    if (g2.getVec(o2, x$1), g2.getVec(r2, E2), w2)
      o$1(D, L[3 * f3 + 0], L[3 * f3 + 1], L[3 * f3 + 2]), r$1(F, D), j.componentIndex = m2.get(o2), j.cosAngle = P(D, F);
    else {
      if (o$1(D, L[3 * f3 + 0], L[3 * f3 + 1], L[3 * f3 + 2]), o$1(F, L[3 * h3 + 0], L[3 * h3 + 1], L[3 * h3 + 2]), j.componentIndex = m2.get(o2), j.cosAngle = P(D, F), A(j, S2))
        continue;
      j.cosAngle < -0.9999 && r$1(F, D);
    }
    O2 += n2, P$1++, w2 || v(j, V2) ? (i.write(k, C++, j), z2.push(n2)) : d$2(j, u2) && (l2.write(K, M++, j), B.push(n2));
  }
  const G = new Float32Array(z2.reverse()), J = new Float32Array(B.reverse());
  return { regular: { instancesData: i.trim(k, C), lodInfo: { lengths: G } }, silhouette: { instancesData: l2.trim(K, M), lodInfo: { lengths: J } }, averageEdgeLength: O2 / P$1 };
}
function v(e2, t2) {
...
const I = { edge: { position0: n$1(), position1: n$1(), faceNormal0: n$1(), faceNormal1: n$1(), componentIndex: 0, cosAngle: 0 }, ortho: n$1(), fwd: n$1() }, V = { v0: n$1(), v1: n$1(), v2: n$1() }, N = { anglePlanar: 4, angleSignificantEdge: 35 };
function t(t2, o2, n2) {
...
function u(i, r2, n2, o2) {
  ...
  const c2 = n$2(i.buffer, i.stride / 4, { originalIndices: n2, originalIndicesLength: o2 }), f2 = t(c2.indices, o2, c2.uniqueCount);

import { ma as containsCrossShadowBoundary, b as closestElementCrossShadowBoundary, p as proxyCustomElement, H, h, c as createEvent, mb as TEXT$1, m7 as getSlotted, i as isPrimaryPointerButton, mc as setRequestedIcon, m6 as getElementProp, g as getElementDir, m8 as CSS_UTILITY, a as Host } from "./index.js";
...
class BigDecimal {
  ...
  toString() {
    const s = this.value.toString().replace(new RegExp("-", "g"), "").padStart(BigDecimal.DECIMALS + 1, "0");
...
function getLocale(component) {
  var _a;
  return component.el.lang || component.locale || ((_a = closestElementCrossShadowBoundary(component.el, "[lang]")) === null || _a === void 0 ? void 0 : _a.lang) || document.documentElement.lang || defaultLocale;
}
class NumberStringFormat {
  constructor() {
    ...
    this.localize = (numberString) => this._numberFormatOptions ? sanitizeExponentialNumberString(numberString, (nonExpoNumString) => isValidNumber(nonExpoNumString.trim()) ? new BigDecimal(nonExpoNumString.trim()).format(this._numberFormatter).replace(new RegExp(`[${this._actualGroup}]`, "g"), this._group) : nonExpoNumString) : numberString;
  }
  get group() {
    return this._group;
  }
...
const Input = /* @__PURE__ */ proxyCustomElement(class extends H {
  constructor() {
    ...
    this.hiddenInputChangeHandler = (event) => {
      if (event.target.name === this.name) {
        ...
        const value = this.type === "number" ? isValidNumber(hiddenInputValue) ? hiddenInputValue : "" : hiddenInputValue;
        this.setValue({
          value,
          origin: "direct"
        });
    ...
    this.setPreviousEmittedValue = (newPreviousEmittedValue) => {
      this.previousEmittedValue = this.type === "number" ? isValidNumber(newPreviousEmittedValue) ? newPreviousEmittedValue : "" : newPreviousEmittedValue;
    };
    this.setPreviousValue = (newPreviousValue) => {
      this.previousValue = this.type === "number" ? isValidNumber(newPreviousValue) ? newPreviousValue : "" : newPreviousValue;
    };
    this.setValue = ({ committing = false, nativeEvent, origin, previousValue, value }) => {
      ...
      if (this.type === "number") {
        const delocalizedValue = this.numberingSystem && this.numberingSystem !== "latn" || defaultNumberingSystem !== "latn" ? numberStringFormatter.delocalize(value) : value;
        const sanitizedValue = sanitizeNumberString(delocalizedValue);
        const newValue = value && !sanitizedValue || [".", "-"].includes(sanitizedValue) ? isValidNumber(this.previousValue) ? this.previousValue : "" : sanitizedValue;
      ...
      if (nativeEvent) {
        ...
        if (calciteInputInputEvent.defaultPrevented) {
          ...
          this.localizedValue = this.type === "number" ? numberStringFormatter.localize(this.previousValue) : this.previousValue;
        } else if (committing) {
          this.emitChangeIfUserModified();
        }
      }
  ...
  incrementOrDecrementNumberValue(direction, inputMax, inputMin, nativeEvent) {
    ...
    const finalValue = typeof inputMin === "number" && !isNaN(inputMin) && nudgedValue < inputMin || typeof inputMax === "number" && !isNaN(inputMax) && nudgedValue > inputMax ? inputVal : nudgedValue;
  ...
  setDisabledAction() {
    ...
    this.disabled ? slottedActionEl.setAttribute("disabled", "") : slottedActionEl.removeAttribute("disabled");
  }
